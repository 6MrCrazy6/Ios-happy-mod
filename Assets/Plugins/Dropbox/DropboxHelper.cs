...
namespace Plugins.Dropbox
{
    public static class DropboxHelper
    {
        ...
        private const string AuthCode = "kF4e5q3jc8YAAAAAAAAAEqhGuyoQBW13CL_x4Q-lgtU";
#endif
        ...
        public static async Task DownloadAndSaveFile(string relativePathToFile)
        {
            ...
            else
            {
                ...
                string filePath = Application.dataPath + "/Resources/" + relativePathToFile;
                System.IO.File.WriteAllBytes(filePath, downloadRequest.downloadHandler.data);
                Debug.Log("File saved to: " + filePath);
            }
        }

        public static UnityWebRequest GetRequestForFolderDownload(string folderName)
        {
            var request = UnityWebRequest.Get("https://content.dropboxapi.com/2/files/download_zip");
            request.SetRequestHeader("Authorization", $"Bearer {_tempRuntimeToken}");
            request.SetRequestHeader("Dropbox-API-Arg", $"{{\"path\": \"/{folderName}\"}}");
            request.downloadHandler = new DownloadHandlerBuffer(); 
            return request;
        }

        public static async Task DownloadAndSaveFolder(string folderName)
        {
            UnityWebRequest downloadRequest = GetRequestForFolderDownload(folderName);
            ...
            if (downloadRequest.result != UnityWebRequest.Result.Success)
            {
                Debug.LogError("Failed to download folder: " + downloadRequest.error);
                return;
            }

            byte[] zipBytes = downloadRequest.downloadHandler.data;

            string saveFolderPath = Path.Combine(Application.dataPath, "Resources", folderName);
            string zipPath = Path.Combine(Application.dataPath, "Resources", folderName + ".zip");

            File.WriteAllBytes(zipPath, zipBytes);

            ZipFile.ExtractToDirectory(zipPath, saveFolderPath);

            File.Delete(zipPath);

            Debug.Log($"Folder downloaded and saved to: {saveFolderPath}");
        }
    }
}